...
class EnumRedisLockTest {
    ...
    public static void lockTest() {
        ...
        if (!EnumRedisLock.INSTANCE.lock(LOCK, EXPIRE)) {
        ...
        EnumRedisLock.INSTANCE.release(LOCK);
        System.out.println("lock test:: end");
    }

    @Test
    void lockTestMe() throws InterruptedException {
        Thread thread1 = new Thread(EnumRedisLockTest::lockTest);
        Thread thread2 = new Thread(EnumRedisLockTest::lockTest);
        thread1.start();
        thread2.start();
        thread1.join();
        thread2.join();
