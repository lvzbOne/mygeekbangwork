...
CREATE TABLE IF NOT EXISTS `shop_goods_repertory`  (
  `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '店铺商品库存物理id自增',
  ...
  `goods_num` int(10) NOT NULL DEFAULT 1 COMMENT '商品库存数量',
  `price` decimal(12, 4) NOT NULL DEFAULT 0.0000 COMMENT '该店铺商品价格',
  `store_date` int(8) NOT NULL COMMENT '入库日期格式yyyyMMdd',
  ...
  UNIQUE INDEX `uk_s_g`(`shop_code`, `goods_code`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '店铺商品库存表' ROW_FORMAT = Dynamic;
